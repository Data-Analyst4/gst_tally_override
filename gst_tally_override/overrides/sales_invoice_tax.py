...
def get_item_tax_template_name(item_code):
    ...
    try:
        ...
        if getattr(item_doc, "taxes", None):
...
def on_before_submit(doc, method):
    ...
    # Validate JSON on the first tax row (standard location for item_wise_tax_detail)
    tax_row = doc.taxes[0] if doc.taxes else None
    if tax_row and getattr(tax_row, "item_wise_tax_detail", None):
        try:
            json.loads(tax_row.item_wise_tax_detail)
...
def apply_normal_invoice_override(doc):
    ...
    # STEP 8: item_wise_tax_detail on tax rows
...
def rebuild_item_wise_tax_detail_from_item_fields(doc):
    """
    Build item_wise_tax_detail in standard format and assign to tax rows.
    Standard ERPNext / IC pattern: {"ITEM-00039": [tax_rate, tax_amount]}.
    """
    item_wise_tax = {}

    for item in doc.items:
        key = item.item_code
        gst_rate, _ = get_item_gst_rate(item, doc)
        # Use IGST amount if inter-state, else CGST+SGST
        ...
        tax_amount = igst if igst else (cgst + sgst)

        item_wise_tax[key] = [gst_rate, tax_amount]

    detail_json = json.dumps(item_wise_tax)

    for tax in doc.taxes:
        tax.item_wise_tax_detail = detail_json

    frappe.logger().debug(f"[GST Override] item_wise_tax_detail: {detail_json}")
